// Модуль C6. API браузера. C6.3. SVG
...
// флаг - показываем первый svg / иначе - второй
let svgFirst = true;

function swapSVG(){
  // Ищем ноду для вставки svg иконки
  const iconNode = document.querySelector('.btn_icon');
  if (svgFirst){
  iconNode.innerHTML=`
  ...
  `;
  svgFirst = false; // меняем флаг, чтобы при следующем вызове показывать другую иконку
  } else {
    iconNode.innerHTML=`
...
// window.onload(swapSVG());
window.addEventListener('load', ()=>{swapSVG()})